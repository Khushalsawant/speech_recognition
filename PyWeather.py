...
def get_weather(query, days=1):
    ...
    for day_weather in data['consolidated_weather'][:days]:
        result['weather'].append({'date': day_weather['applicable_date'],'min_temp': round(day_weather['min_temp'], 2),'max_temp': round(day_weather['max_temp'], 2),'weather_state_name': day_weather['weather_state_name']})
    return result
...
def print_weather_details(data):
    ...
    for row in data['weather']:
        ...
        print('-'*50)
weather_data  = get_weather(query='Mumbai')
print_weather_details(weather_data )
weather_data_yesterday = get_weather(query='Mumbai',days=2)
print_weather_details(weather_data_yesterday)